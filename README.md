...
- 🗺️ **四套底图切换**：OSM 标准 / 高德街道 / Stamen 地形 / 卫星影像
...
- 📷 **照片画廊与灯箱**：点击门店照片查看全屏大图，支持滚轮缩放、拖拽平移、双击放大、触摸双指缩放、键盘导航
- 🌐 **繁简切换**：一键切换简体中文 / 繁体中文界面，语言偏好自动保存
- 📂 **面板折叠**：桌面端和移动端均可折叠/展开顶部控件栏
- 📝 **文字地址支持**：地址字段支持纯文字描述，不仅限经纬度坐标
- 📱 **响应式设计**：手机端自动适配，弹窗字体使用 clamp() 自适应缩放
- 💾 **地图状态持久化**：自动保存地图视角、缩放级别、底图偏好
...
| `photo_url` | string | ❌ | 当前门店老照片链接（单张，可为空） | `https://...`/`http://...` |
| `photo_urls` | array | ❌ | 当前门店老照片链接数组（多张） | `["https://...", "https://..."]` |
...
| `photo_url` | string | ❌ | 该位置的老照片链接（单张） |
| `photo_urls` | array | ❌ | 该位置的老照片链接数组（多张） |
